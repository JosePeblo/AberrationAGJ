...
void Shader::SetUniform1i(const std::string& name, int value)
{
    ...
    if(location != -1)
    {
        m_uniformLocationCache[name].type = UNIFORM_TYPE::UNIFORM_INT;
...
void Shader::SetUniform2i(const std::string& name, ivec2& value) {
    GLint location = GetUniformLocation(name);
    if(location != -1)
    {
        m_uniformLocationCache[name].type = UNIFORM_TYPE::UNIFORM_IVEC2;

        void* data = m_uniformLocationCache[name].data;
        if(!data)
            data = new ivec2(value);
        else
            *((ivec2*)data) = value;
...
void Shader::SetUniform1f(const std::string& name, float value)
{
    ...
    if(location != -1)
    {
        m_uniformLocationCache[name].type = UNIFORM_TYPE::UNIFORM_FLOAT;
...
void Shader::SetUniform2f(const std::string& name, const vec2& value)
{
    ...
    if(location != -1)
    {
        m_uniformLocationCache[name].type = UNIFORM_TYPE::UNIFORM_VEC2;
...
void Shader::SetUniform3f(const std::string& name, const vec3& value)
{
    ...
    if(location != -1)
    {
        m_uniformLocationCache[name].type = UNIFORM_TYPE::UNIFORM_VEC3;
...
void Shader::SetUniform4f(const std::string& name, const vec4& value)
{
    ...
    if(location != -1)
    {
        m_uniformLocationCache[name].type = UNIFORM_TYPE::UNIFORM_VEC4;
...
uint32_t ComputeShader::CreateShader(const std::string& source)
{
